...
class Test_Multisite_Handler extends WP_UnitTestCase {
	...
	public function test_multi_label_subdomain_sanitized_to_dashed_slug(): void {
		// news.greg.harmsboone.org would leave slug "news.greg" — dots replaced with dashes.
		$this->assertSame(
	...
	public function test_base_domain_not_stripped(): void {
		// harmsboone.org itself does not end with .harmsboone.org, so no stripping; dots replaced.
		$this->assertSame(
			'/harmsboone-org/',
	...
	public function test_no_network_domain_falls_back_to_full_host(): void {
		// Without network_domain, dots in hostname are replaced with dashes.
		$this->assertSame(
			'/greg-harmsboone-org/',
